...
import { comapnySignupApi } from '../../../apis/CompanyApi';

const theme = createTheme();

export default function CompanyOTP() {
  ...
  const resendOTP = () => {
    setCaptchaDiv(false);
    setUpRecaptcha(`+91${companyDetails.phoneNumber}`).then((res) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (data.otp === '' || data.otp === null) {
      ...
      try {
        await companyOtpConf.confirm(data.otp);
        const response = await comapnySignupApi(companyDetails);
                ...
                {captchaDiv ? ''
                  : (
                    <Grid item xs={12} sx={{ px: 2 }}>
                      <div style={{ marginTop: '5px' }} id="recaptcha-company-container" />
                    </Grid>
                  )}
